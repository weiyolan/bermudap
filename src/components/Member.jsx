...
export default function Member({ url, name, func, text, alt, print, className, index, }) {
  ...
  return (
    <div
      ...
      tabIndex="0"
      className={` flex absolute cursor-pointer flex-col items-center mt-2 select-none left-1/2 top-0 -translate-x-1/2   ${className}`}
    >

      <div ref={myImgContainer} className="relative mb-2 select-none h-48 w-48 overflow-hidden bg-brown rounded-full ">
        <Image
          ref={myImg}
          alt={alt}
          src={url}
      ...
      <p ref={myText} className="absolute italic opacity-0 top-1/3 select-text -translate-y-1/2  w-full text-center font-raj font-medium" >
        {'"'}{text}{'"'}
      </p>
